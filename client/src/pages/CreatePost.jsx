  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-6xl mx-auto p-6">
        ...
        {/* Form  */}
        <div className="max-w-xl bg-white p-4 sm:p-8 sm:pb-8 rounded-xl shadow-md space-y-4">
          {/* header  */}
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h2 className="font-sembold">{user.full_name}</h2>
              <p className="text-sm text-gray-500">@{user.username}</p>
            </div>
          </div>
          {/* Text Area  */}
          <textarea className="w-full resize-non max-h-20 mt-4 text-sm outline-none placeholder-gray-400" placeholder="what's happening"
            ...
            images.length > 0 && <div className="flex flex-wrap gap-2 mt-4">
              {
                images.map((image, i)=>(
                  ...
                  <div onClick={()=>setImages(images.filter((_, index)=> index!==i))} className="absolute hidden group-hover:flex justify-center items-center top-0 right-0 bottom-0 left-0 bg-black/40
